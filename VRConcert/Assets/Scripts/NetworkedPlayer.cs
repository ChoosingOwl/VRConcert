...
public class NetworkedPlayer : Photon.MonoBehaviour {
    ...
    void Start () {
        ...
        if (photonView.isMine)
        {
            ...
            {
                spawnedSphere = (GameObject)Instantiate(Resources.Load("Sphere"), new Vector3(100f, 0f, 0f), Quaternion.identity);

                //Instantiates the camera rigs here that combines the avatar with the rig. 
                spawnedCameraRig = (GameObject)Instantiate(Resources.Load("[CameraRig]"), new Vector3(100f, 0f, 0f), Quaternion.identity);
                playerGlobal = GameObject.Find("Player2(Clone)").transform;
                playerLocal = playerGlobal.Find("[CameraRig]/Camera (head)/Camera (eye)");
            }
